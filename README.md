...
- AB Test different Ad configurations.
- Use segmentation to personalize Ad configuration for different player groups .
- Analyze and Report on the impact of different Ad configurations.
...
## Code
If you are displaying Ads in your game, it will already contain a line of code that makes a call to an Ad Network's SDK to Show and Ad at an appropriate time (the placement). 

In order to support the personalisation techniques described above, all you need to do is to move this line code from it's current position and place it inside the callback that fires when an Event Triggered Campaign containing game parameters is triggered. 
...
The actual rewarding of currency to the player is done inside a Listener from the Ad Network SDK. It checks that the player has watched enough of the Ad, issues the reward based on the **adRewardValue** retrieved above, then records a deltaDNA event to provide reporting.
...
Notice the *Per-session execution limit* this can be used to limit the number of Ads the player sees per session, we haven't put a limit on this.
...
### Conversion Event
A conversion event hasn't been specified in this campaign as we want to the campaign to run until we stop it. 
...
The popup Image Message contains an image and a button that have been uploaded to deltaDNA. It has been configured to pass an action named "Watch Video" to the Image Message Callback, or dismiss the popup if the player clicks the image background. 
...
They will be shown a popup image message encouraging them to visit the store and purchase a new player bundle containing 1000 coins. 
![Out Of Coins New Player Segment](Images/OutOfCoins-IAP-Action.jpg)
The popup Image Message contains an image and a button that have been uploaded to deltaDNA. It has been configured to navigate the player to the App Store if they click the button, or dismiss the popup if they click the image background. 
...
## Conclusion
It really is that simple! With just a few line of code and some simple campaigns  it is possible to completely personalize your Ad strategy. The great thing is that you can measure the success and refine your implementation remotely without haing to make changes to your game and go back through the sumbisison process each time.  
